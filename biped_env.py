...
class BipedEnv:
    ...
    def step(self, actions):
        ...
        # Apply threshold for binary contact detection
        # Use configurable threshold for binary contact detection
        contact_threshold = self.env_cfg.get("binary_contact_threshold", 0.1)  # Default to 0.1 N if not specified
        foot_contacts_binary = (self.foot_contacts > contact_threshold).float()  # Binary: 0 or 1
        ...
        if self.env_cfg["domain_rand"]["add_observation_noise"] and self._should_update_randomization('observation_noise'):
            ...
            # Add noise to foot contact readings (binary contacts with potential noise)
            if self.noise_buffers['foot_contact'].numel() > 0:
                # Apply noise to binary contacts and re-threshold
                foot_contacts_with_noise = foot_contacts_binary + self.noise_buffers['foot_contact']
                foot_contacts_noisy = (foot_contacts_with_noise > 0.5).float()  # Re-binarize after noise
            else:
                foot_contacts_noisy = foot_contacts_binary
        else:
    ...
    # def _reward_sinusoidal_gait(self):
        ...
        leg_joints_default = self.default_dof_pos  # All joints (no torso in current config)
        target_leg_pos = leg_joints_default + amplitude * torch.sin(
            2 * np.pi * frequency * time + phase_offsets
        )

        leg_joints_current = self.dof_pos  # All joints
    ...
    # def _reward_torso_sinusoidal(self):
        # Torso sinusoidal reward is disabled when no torso joint exists
        # Return zero reward since current config has no torso joint
        if self.num_actions < 9:  # No torso joint in current 8-joint configuration
